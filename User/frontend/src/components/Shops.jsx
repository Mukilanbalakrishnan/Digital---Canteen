            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {shops.map((shop, index) => (
                    <div
                        key={index}
                        onClick={() => handleShopClick(shop.shopName)}
                        className="bg-blue-200 p-5 rounded-xl shadow-md hover:shadow-xl cursor-pointer border:blur transition"
                    >
                        <h3 className="text-xl font-semibold text-indigo-800">{shop.shopName}</h3>
                        <p className="text-gray-600 mt-1 font-semibold">Owner: <span className="font-bold text-orange-700">{shop.shopOwnerName}</span></p>
                    </div>
                ))}
            </div>
